...
function extractIndeedJob() {
	const company =
		...
		document
			.querySelector(".jobsearch-CompanyInfoContainer")
			?.textContent?.trim();

	const position =
		document
			.querySelector('[class*="jobsearch-JobInfoHeader-title"]')
			?.textContent?.trim() ||
		document
...
function extractLeverJob() {
	const company = document
		.querySelector(".main-header-text-logo")
		?.textContent?.trim();
	...
	const description =
		document.querySelector(".posting-description")?.textContent?.trim() ||
...
function extractGeneric() {
	...
	const position =
		document.querySelector("h1")?.textContent?.trim() || ogTitle || pageTitle;
