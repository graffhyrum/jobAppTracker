...
function initializePipelineClient(applicationData: ProcessedApplication[], config: PipelineConfig) {
...
const setupEventListeners = (applicationData: ProcessedApplication[], state: PipelineState): void => {
    ...
    document.body.addEventListener("htmx:beforeRequest", (evt: CustomEvent<HtmxBeforeRequestDetail>) => {
        console.log("HTMX: Starting request to", evt.detail.requestConfig.url);
    });
};
const filterData = (applicationData: ProcessedApplication[], state: PipelineState, immediate = true): void => {
    ...
    const interestSelect = document.getElementById("interest-filter") as HTMLSelectElement;
    const overdueSelect = document.getElementById("overdue-filter") as HTMLSelectElement;
...
const updateDisplay = (state: PipelineState): void => {
    const sortedData = sortData(state.filteredData, state.currentSort.column as keyof ProcessedApplication, state.currentSort.direction);
...
const sortData = (data: ProcessedApplication[], column: keyof ProcessedApplication, direction: "asc" | "desc"): ProcessedApplication[] => {
