...
import type { ApplicationStatus, JobApplication, JobApplicationForCreate, } from "#src/domain/entities/job-application.ts";
...
import { type createApplicationBodySchema, updateApplicationBodySchema, } from "#src/presentation/schemas/application-routes.schemas.ts";
type ApplicationStatusLabel = typeof jobApplicationModule.ApplicationStatusLabel.infer;
...
// Create schema once at module initialization to avoid race conditions under concurrent load
const dateNormalizeSchema = type("string.date.iso").pipe((md) => new Date(md).toISOString());
/**
 * Transforms form data for PUT request, handling status field conversion
 */
export const transformUpdateData = (body: unknown, currentApp: JobApplication | null) => {
    ...
    if ("status" in formData && typeof formData.status === "string") {
        if (currentApp) {
            const statusLabel = formData.status as typeof jobApplicationModule.ApplicationStatusLabel.infer;
 ...
 */
export const extractApplicationData = (formData: typeof createApplicationBodySchema.infer): JobApplicationForCreate => {
    ...
    if (!company || !positionTitle || !applicationDate) {
        throw new Error("Company, position title, and application date are required");
    }
    ...
    const isRemote = isRemoteValue === true ||
        isRemoteValue === "true" ||
    ...
    const data: JobApplicationForCreate = {
        ...
        sourceType: typeof formData.sourceType === "string" &&
 ...
 */
export const createApplicationStatus = (label: ApplicationStatusLabel): ApplicationStatus => {
 ...
 */
export const extractStringField = (value: unknown, defaultValue: string | undefined = ""): string | undefined => {
 ...
 */
export const fetchAllApplicationsOrEmpty = async (manager: JobApplicationManager): Promise<JobApplication[]> => {
