import type { InterviewStage, InterviewType, } from "../entities/interview-stage.ts";
 ...
 */
export function computeInterviewAnalytics(applications: JobApplication[], interviewStages: InterviewStage[]): InterviewAnalytics {
    ...
    return {
        ...
        interviewConversionRate: computeInterviewConversionRate(applications, interviewsByApp),
        averageDaysFromApplicationToFirstInterview: computeAverageDaysToFirstInterview(applications, interviewsByApp),
        medianDaysFromApplicationToFirstInterview: computeMedianDaysToFirstInterview(applications, interviewsByApp),
        averageDaysBetweenRounds: computeAverageDaysBetweenRounds(interviewsByApp),
        interviewTypeEffectiveness: computeInterviewTypeEffectiveness(applications, interviewStages, interviewsByApp),
        roundAnalysis: computeRoundAnalysis(applications, interviewStages, interviewsByApp),
        finalRoundSuccess: computeFinalRoundSuccess(applications, interviewStages, interviewsByApp),
        interviewCompletionRate: computeInterviewCompletionRate(interviewStages),
    };
}
function computeAverageDaysBetweenRounds(interviewsByApp: Map<string, InterviewStage[]>): number {
...
function computeMedianDaysToFirstInterview(applications: JobApplication[], interviewsByApp: Map<string, InterviewStage[]>): number {
...
function computeAverageDaysToFirstInterview(applications: JobApplication[], interviewsByApp: Map<string, InterviewStage[]>): number {
    const daysToFirstInterview: number[] = [];
    for (const app of applications) {
        const interviews = interviewsByApp.get(app.id) ?? [];
        if (interviews.length > 0) {
...
function computeRoundsToOffer(applications: JobApplication[], interviewsByApp: Map<string, InterviewStage[]>): number[] {
...
function computeInterviewConversionRate(applications: JobApplication[], interviewsByApp: Map<string, InterviewStage[]>): number {
...
function computeInterviewTypeEffectiveness(applications: JobApplication[], interviewStages: InterviewStage[], _interviewsByApp: Map<string, InterviewStage[]>): InterviewTypeEffectiveness[] {
    const typeMap = new Map<InterviewType, {
        total: number;
        offers: number;
        rejected: number;
    }>();
...
function computeRoundAnalysis(applications: JobApplication[], interviewStages: InterviewStage[], interviewsByApp: Map<string, InterviewStage[]>): RoundAnalysis[] {
...
function computeFinalRoundSuccess(applications: JobApplication[], interviewStages: InterviewStage[], _interviewsByApp: Map<string, InterviewStage[]>): FinalRoundSuccess {
    const finalRounds = interviewStages.filter((i) => i.isFinalRound);
    const appIdsWithFinalRound = new Set(finalRounds.map((i) => i.jobApplicationId));
    let offers = 0;
    let rejections = 0;
...
function computeInterviewCompletionRate(interviewStages: InterviewStage[]): InterviewCompletionRate {
    let scheduled = 0;
    let completed = 0;
    for (const interview of interviewStages) {
