...
const jobApplicationScope = scope({
    ...noteScope.export(),
    ActiveLabels: "'applied'|'screening interview'|'interview'|'onsite'|'online test'|'take-home assignment'|'offer'",
    InactiveLabels: "'rejected'|'no response'|'no longer interested'|'hiring freeze'",
    "#ActiveState": {
        category: "'active'",
        label: "ActiveLabels",
    },
...
export type JobApplicationForCreate = typeof jobApplicationModule.forCreate.infer;
export type JobApplicationForUpdate = typeof jobApplicationModule.forUpdate.infer;
export type JobApplicationId = typeof jobApplicationModule.JobAppId.infer;
export type ApplicationStatus = typeof jobApplicationModule.ApplicationStatus.infer;
...
export type ApplicationStatusLabel = typeof jobApplicationModule.ApplicationStatusLabel.infer;
...
export function getStatusCategory(app: JobApplication): Result<"active" | "inactive", ArkErrors> {
    return getCurrentStatus(app).map((status) => status.category);
}
export function getCurrentStatus(app: JobApplication): Result<ApplicationStatus, ArkErrors> {
    const statusEntry = getJobAppCurrentStatusEntry(app);
    return statusEntry.map(([_, status]) => status);
}
export function createJobApplicationWithInitialStatus(data: JobApplicationForCreate, generateUUID: () => string): JobApplication {
...
export function createJobApplicationWithIdGen(data: JobApplicationForCreate, generateUUID: () => string): JobApplication {
    const id = createJobApplicationId(generateUUID);
    return createJobApplication(data, id);
}
function _assertIsJobApplicationForCreate(x: unknown): asserts x is JobApplicationForCreate {
    jobApplicationModule.forCreate.assert(x);
}
export function createJobApplicationId(generateUUID: () => string): JobApplicationId {
    return jobApplicationModule.JobAppId.assert(generateUUID());
}
export function getJobAppCurrentStatusEntry(app: JobApplication): Result<AppStatusEntry, ArkErrors> {
...
export function createJobApplication(data: JobApplicationForCreate, id: JobApplicationId): JobApplication {
...
export function updateJobApplicationStatus(app: JobApplication, newStatus: ApplicationStatus): JobApplication {
    const now = new Date().toISOString();
...
export function updateJobApplicationData(app: JobApplication, updates: Partial<Omit<JobApplication, "id">>): JobApplication {
    return {
        ...app,
        ...updates,
