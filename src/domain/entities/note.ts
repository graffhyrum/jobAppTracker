...
import { type Note, type NoteCollection, type NoteForCreate, type NoteId, type NoteProps, type NotesCollection, noteModule, } from "./noteScope.ts";
export function createNotesCollectionManager(generateId: () => NoteId, collection: NoteCollection = {}): NotesCollection {
    return {
        notes: collection,
        operations: {
            get(id: NoteId) {
            ...
            getAll(): Result<Note[], Error> {
                ...
                if (maybeNotes.length > 0) {
                    return ok(maybeNotes.map(([id, data]) => {
                        return { id, ...data };
                    }));
                }
                else {
...
export function createNote({ content, }: NoteForCreate): Result<NoteProps, Error> {
